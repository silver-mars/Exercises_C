...
**kubernetes get po -n my-namespace
kubernetes logs my-service-blablabla-blabla -n my-namespace**
...
**kubernetes -n my-namespace get po
kubernetes -n my-namespace logs my-service-blablabla-blabla**
...
**grep <notextile>-</notextile>C 50 repproduced /opt/srv/logs/srv_important.log**
...
Но, используя мозг, можно переписать всю команду следующим образом:
...
**history | grep cat** (подробнее про манипуляции с введёнными ранее командами, history и её переменные см. [http://rus-linux.net/MyLDP/BOOKS/Linux_Foundations/14/ch14.html?ysclid=lcad7ang48401895137](rus-linux.net)
"rus-linux.net":http://rus-linux.net/MyLDP/BOOKS/Linux_Foundations/14/ch14.html?ysclid=lcad7ang48401895137
...
Теперь вам достаточно назвать этот скрипт с какой-нибудь уникальной буквы (чтобы после неё можно было сразу добивать tab'ом всё название, в этом примере я назову srv_log.sh) и написать в командной строке
**bash s<tab> <слово для поиска>**
...
{{collapse()
kafkacat -C -b **gitlab-ci-sand-awesome.ru:9092** -t confirmed -o beginning
kafkacat -C -b **test-gitlab-ci-node-awesome.ru** -t confirmed -o beginning
kafkacat -C -b **prod-gitlab-ci-node-amazing.ru:9092** -t confirmed -o beginning
или
/opt/kafka/kafka/bin/kafka-console-consumer.sh --bootstrap-server **15.83.178.212:9092** --group node-integrator --topic test-node-response
/opt/kafka/kafka/bin/kafka-console-consumer.sh --bootstrap-server **16.84.179.213:9092** --group node-integrator --topic test-node-response
/opt/kafka/kafka/bin/kafka-console-consumer.sh --bootstrap-server **17.85.180.214:9092** --group node-integrator --topic test-node-response
}}
...
**Примечание:** можно указывать только дату. Подробнее про манипуляции со временем вы можете посмотреть, например, здесь:
[https://www.dmosk.ru/miniinstruktions.php?mini=search-linux&ysclid=lcowsa1cy7943112085](dmosk.ru/miniinstruktions)

"dmosk.ru/miniinstruktions":https://www.dmosk.ru/miniinstruktions.php?mini=search-linux&ysclid=lcowsa1cy7943112085
